...
class VTeacher extends StatefulWidget {
  const VTeacher({super.key});

  @override
  State<VTeacher> createState() => _VTeacherState();
}

class _VTeacherState extends State<VTeacher>
    with SingleTickerProviderStateMixin {
  ApiTeacher api = ApiTeacher();
  ...
  late List<Teacher> _teacherData;
  @override
  void initState() {
      ...
      child: CustomScrollView(
        slivers: <Widget>[
          const SliverPersistentHeader(
            delegate: SliverSearchAppBar("Teacher(s)"),
            pinned: true,
          ),
          loadFutureList()
        ],
                            ...
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                  ...
                                  setState(() {
                                    _teacherData = [];
                                    _future = api
                                        .advanceSearch(_searchController.text);
                                  }),
                                })
                          },
      ...
      builder: (BuildContext context, AsyncSnapshot<List<Teacher>?> snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasError) {
            ...
            _teacherData = snapshot != null && snapshot.data != null
                ? snapshot.data! ?? []
                : [];
            return SliverToBoxAdapter(
                child: Container(
  ...
  listObject() {
    return (_teacherData.isNotEmpty
        ? _teacherData
            .map((teacher) => Padding(
                  padding:
                  ...
                  child: SecondaryCourseCard(
                    ...
                    routeName: "dteacher",
                    highlightImage: teacher.photo != null
                        ? base64Decode(teacher.photo.toString())
                        : null,
                    title:
                        "${teacher.firstname} ${teacher.middlename} ${teacher.lastname}",
                    subtitle: teacher.mobileno,
                    iconsSrc: "$assetsLocation/icons/teachers.svg",
                    data: teacher,
                  ),
                ))
            .toList()
